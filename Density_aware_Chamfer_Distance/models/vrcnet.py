...
class MSAP_SKN_decoder(nn.Module):
    def __init__(self, num_coarse_raw, num_fps, num_coarse, num_fine, layers=[2, 2, 2, 2], knn_list=[10, 20], pk=10,
                 points_label=False, local_folding=False, over_scale=1):
    ...
    def forward(self, x, gt, is_training=True, mean_feature=None, alpha=None, test_emd=True):
        ...
        if is_training:
            ...
            if self.train_loss == 'cd':
                ...
                loss1, _, _ = calc_dcd(coarse_raw, gt, alpha=t_alpha * 2, n_lambda=n_lambda)
                ...
                loss4, _, _, dist1, dist2, idx1, idx2 = calc_dcd(fine, gt, alpha=t_alpha, return_raw=True, n_lambda=n_lambda)
            else:
            ...
            return fine, loss4, total_train_loss, None
        else:
