...
class Model(nn.Module):
    ...
    def forward(self, x, gt, is_training=True, mean_feature=None, alpha=None, test_emd=True, ind=0):
        num_input = x.size()[2]
        ...
        if is_training:
            ...
            elif self.train_loss == 'reverse':
                loss1, _,  c_dist1, c_dist2, c_idx1, c_idx2 = calc_reverse_cd(coarse_raw, gt, return_raw=True)
                loss2, _ = calc_reverse_cd(coarse_high, gt)
                loss3, _ = calc_reverse_cd(coarse, gt)
                loss4, _, dist1, dist2, idx1, idx2 = calc_reverse_cd(fine, gt, return_raw=True)
            elif self.train_loss == 'uniform':
                loss1, _,  c_dist1, c_dist2, c_idx1, c_idx2 = calc_uniform_cd(coarse_raw, gt, return_raw=True)
                loss2, _ = calc_uniform_cd(coarse_high, gt)
                loss3, _ = calc_uniform_cd(coarse, gt)
                loss4, _, dist1, dist2, idx1, idx2 = calc_uniform_cd(fine, gt, return_raw=True)
            ...
            # with open ("outputs/fine" + str(ind) + ".pkl", "wb") as f:
            #     pickle.dump([fine, x.transpose(1,2), gt], f)
            # guided down-sampling
            if self.eval_emd and test_emd:
                emd = calc_emd(fine, gt, eps=0.004, iterations=300)
            else:
                emd = torch.ones(1).cuda()
            cd_p, cd_t, f1 = calc_cd(fine, gt, calc_f1=True)
            dcd, _, _ = calc_dcd(fine, gt)
            bcd_p, bcd_t = calc_uniform_cd(fine, gt)
            #res = {'out1': coarse_raw, 'out2': fine, 'emd': emd, 'dcd': dcd, 'cd_t': cd_t, 'f1': f1}
            res = {'out1': coarse_raw, 'out2': fine, 'emd': emd, 'dcd': dcd, 'cd_t': cd_t, 'f1': f1, 'bcd_t': bcd_t}
