...
class Model(nn.Module):
    ...
    def forward(self, x, gt=None, is_training=True, alpha=None, test_emd=False):
        feat = self.encoder(x)
        ...
        # if not is_training:
        #     # print("fine shape: ", out2.shape, x.shape)
        #     # with open ("fine.pkl", "wb") as f:
        #     #     pickle.dump([out2, x.transpose(1,2)], f)
        ...
        if is_training:
            if self.train_loss == 'emd':
                ...
                loss1, _, _ = calc_dcd(out1, gt, alpha=t_alpha, n_lambda=n_lambda)
                loss2, _, _ = calc_dcd(out2, gt, alpha=t_alpha, n_lambda=n_lambda)
            elif self.train_loss == 'uniform':
                loss1, _ = calc_uniform_cd(out1, gt)
                loss2, _ = calc_uniform_cd(out2, gt)
            ...
            return out2, loss2, total_train_loss, None
            ...
            dcd, _, _ = calc_dcd(out2, gt)

            res = {'out1': out1, 'out2': out2, 'emd': emd, 'dcd': dcd, 'cd_t': cd_t, 'f1': f1}
