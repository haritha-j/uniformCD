...
def calc_cd(output, gt, calc_f1=False, return_raw=False, normalize=False, separate=False):
    ...
    if return_raw:
        ...
        #print("dist1", dist1.shape, dist2.shape, idx1.shape, idx2.shape)
    return res



def calc_uniform_cd(output, gt, calc_f1=False, return_raw=False, normalize=False, separate=False):
    # cham_loss = dist_chamfer_3D.chamfer_3DDist()
    ...
    nn = chamferDist(
        output, gt, bidirectional=True, return_nn=True, k=k
    )

    # measure density with itself
    nn_x = chamferDist(output, output, bidirectional=False, return_nn=True, k=k2)
    ...
    # measure density with itself
    nn_y = chamferDist(gt, gt, bidirectional=False, return_nn=True, k=k2)
    ...
    cd_p = (torch.sqrt(min_dist_1).mean(1) + torch.sqrt(min_dist_0).mean(1)) / 2
    cd_t = (min_dist_1.mean(1) + min_dist_0.mean(1))
    res = [cd_p, cd_t]
    if calc_f1:
        f1, _, _ = fscore(min_dist_1, min_dist_0, 0.0001)
        res.append(f1)

    if return_raw:
        res.extend([min_dist_1, min_dist_0, i1, i0])
        #print("dist2", min_dist_1.shape, min_dist_0.shape, i1.shape, i2.shape)

    return res


def calc_reverse_cd(output, gt, calc_f1=False, return_raw=False, normalize=False, separate=False):
    k=32
    chamferDist = ChamferDistance()
    ...
    _, i1 = torch.min(scaled_dist_1, 2)
    ...
    _, i2 = torch.min(scaled_dist_0, 2)
    ...
    if return_raw:
        res.extend([min_dist_1, min_dist_0, i1, i2])
        #print("dist2", min_dist_1.shape, min_dist_0.shape, i1.shape, i2.shape)
