...
def val(net, curr_epoch_num, val_loss_meters, dataloader_test,best_epoch_losses=None,
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(dataloader_test)):
            ...
            gt = gt.float().cuda()
            inputs = inputs.transpose(2, 1).contiguous()
            result_dict = net(inputs, gt, is_training=False, test_emd=test_emd, ind=i)
            #print("result", result_dict["emd"], result_dict["cd_t"])
            cds.append(result_dict["cd_t"])
            emds.append(result_dict["emd"])
        ...
        # with open ("out_metrics_cd.pkl", "wb") as f:
        #     pickle.dump([cds, emds], f)

        if eval_cat:
            logger.info('Loss per category:')
            category_log = ''
            scalars = dict(cd_t=10000, emd=100, dcd=1, f1=100, bcd_t=10000)
            # print(test_cat_num)
